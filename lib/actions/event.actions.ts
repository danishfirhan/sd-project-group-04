...
slug: string;
venue: string;
ticketPrice: number; // Assuming this is a numeric value
...
try {
    ...
    venue: eventData.venue,
    ticketPrice: eventData.ticketPrice?.toString(), // Convert number to string if provided
    availableTickets: eventData.availableTickets,
    }).where(eq(events.id, id));
